		...
		try {
			...
			for (let i = 0; i < rows.length; i++) {
				...
				await db.insert(users).values({
					...
					id_card,
					password_hash,
					agency_id: row['รหัสหน่วยงาน'] ? Number(row['รหัสหน่วยงาน']) : null,
					...
					hire_type_id: row['รหัสประเภทบรรจุ'] ? Number(row['รหัสประเภทบรรจุ']) : null,
					division_id: row['รหัสแผนก'] ? Number(row['รหัสแผนก']) : null,
					must_change_password: true,
					profile_completed: true
				});
